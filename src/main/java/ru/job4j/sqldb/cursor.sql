...
insert into products_1 (name, count, price)
...
BEGIN TRANSACTION;
DECLARE cur_products SCROLL CURSOR FOR SELECT * FROM products_1;

 		MOVE LAST FROM cur_products;
		FETCH LAST FROM cur_products; -- last row
		
		MOVE BACKWARD 6 FROM cur_products;
		FETCH FROM cur_products; -- 15 row
		
		MOVE BACKWARD 9 FROM cur_products;
		FETCH FROM cur_products; -- 7 row
		
		MOVE BACKWARD 6 FROM cur_products;
		FETCH  FROM cur_products; -- 2 row
		
		MOVE FIRST FROM cur_products;
		FETCH FIRST FROM cur_products; -- 1st row
